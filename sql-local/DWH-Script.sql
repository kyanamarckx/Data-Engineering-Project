...
SELECT DISTINCT airline_iata_code, airline_name, country FROM groep8dep.airline WHERE airline_iata_code NOT IN (SELECT DISTINCT airline_iata_code FROM DimAirline);
...
INSERT INTO DimAirport(airport_iata_code, airport_name, location, country)
SELECT DISTINCT airport_iata_code, airport_name, location, country FROM groep8dep.airport WHERE airport_iata_code NOT IN (SELECT DISTINCT airport_iata_code FROM DimAirport);
